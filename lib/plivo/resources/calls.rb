  ...
  module Resources
    ...
    class Call < Base::Resource
      ...
      def update(options)
        ...
        [:aleg_method, :bleg_method].each do |param|
      ...
      def record(options = nil)
        ...
        params = {}
        [:transcription_url, :callback_url].each do |param|
          if options.key?(param) &&
        ...
        [:transcription_method, :callback_method].each do |param|
          if options.key?(param) &&
             valid_param?(param, options[param], [String, Symbol], true, %w[GET POST])
            params[param] = options[param]
          end
    ...
    class CallInterface < Base::ResourceInterface
      ...
      def list(options = nil)
        ...
        [:offset, :limit].each do |param|
          if options.key?(param) && valid_param?(param, options[param], [Integer, Fixnum], true)
            params[param] = options[param]
          end
        end
